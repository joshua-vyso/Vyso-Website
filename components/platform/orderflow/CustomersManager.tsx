...
export function CustomersManager({ customers }: { customers: OfCustomer[] }) {
  ...
  async function save() {
    ...
    const { error: saveErr } =
      editing === 'new'
        ? await supabase.from('of_customers').insert({ org_id: org.id, ...payload })
        : editing
          ? await supabase.from('of_customers').update(payload).eq('id', editing)
          : { error: null };
    setBusy(false);
    if (saveErr) {
      // Keep the editor open so the typed values aren't lost.
      setError(isUniqueViolation(saveErr) ? 'A customer with that name already exists.' : saveErr.message);
      return;
    }
    setEditing(null);
    router.refresh();
  ...
  const field = 'h-10 w-full rounded-lg border border-[#EAEDF2] bg-white px-3 text-[14px] text-[#171A17] placeholder:text-[#8A8E86] focus:border-[#3E7BC4]/40 focus:outline-none';

  const form = (
    <div className="rounded-2xl border border-[#3E7BC4]/30 bg-[#FBFCFE] p-4">
      ...
      <div className="mt-3 flex justify-end gap-2">
        <button type="button" onClick={() => setEditing(null)} className="rounded-lg px-3.5 py-2 text-[13px] text-[#6B6F68] hover:bg-black/[0.03]">
          Cancel
        </button>
        <button
          type="button"
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-[26px] font-bold text-[#171A17]">Customers</h1>
          <p className="mt-1 text-[14px] text-[#6B6F68]">Profiles, pricing agreements and per-customer parameters</p>
        </div>
        {editing === null ? (
          <button
            type="button"
      ...
      <div className="mt-6 space-y-3">
        ...
        {customers.length === 0 && editing !== 'new' ? (
          <div className="rounded-2xl border border-dashed border-[#EAEDF2] bg-white px-8 py-14 text-center">
            <h2 className="text-[18px] font-semibold text-[#171A17]">No customers yet</h2>
            <p className="mx-auto mt-1 max-w-md text-[14px] text-[#6B6F68]">Add your first customer to start building orders.</p>
          </div>
        ) : (
          customers.map((c) =>
            editing === c.id ? (
              ...
              <div
                key={c.id}
                className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#EAEDF2] bg-white px-5 py-4"
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-[15px] font-semibold text-[#171A17]">{c.name}</span>
                    <span
                  ...
                  {c.notes ? <div className="mt-0.5 text-[12px] text-[#8A8E86]">{c.notes}</div> : null}
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  {confirmDel === c.id ? (
                    <>
                      <span className="text-[12px] text-[#6B6F68]">Delete?</span>
                      <button type="button" onClick={() => setConfirmDel(null)} className="rounded-lg px-2.5 py-1.5 text-[13px] text-[#6B6F68] hover:bg-black/[0.03]">
                        No
                      </button>
                    ...
                    <>
                      <button type="button" onClick={() => startEdit(c)} className="rounded-lg border border-[#EAEDF2] bg-white px-3 py-1.5 text-[13px] text-[#171A17] transition-colors hover:border-[#3E7BC4]/30">
                        Edit
                      </button>
                      <button type="button" onClick={() => setConfirmDel(c.id)} className="rounded-lg px-2.5 py-1.5 text-[13px] text-[#8A8E86] transition-colors hover:bg-[#FCEBEB] hover:text-[#A32D2D]">
                        Delete
                      </button>
                    </>
                  )}
