...
export function InvoicingView({
  ...
  return (
    <div>
      {toastNode}
      <div>
        <h1 className="text-[26px] font-bold text-[#171A17]">Invoicing</h1>
        <p className="mt-1 text-[14px] text-[#6B6F68]">Issue invoices, track payments and stay on top of what&rsquo;s owed</p>
      </div>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2">
        <input
          ...
          className="h-9 min-w-[240px] flex-1 rounded-lg border border-[#E2E6EC] bg-white px-3 text-[13px] text-[#171A17] outline-none placeholder:text-[#8A8E86] focus:border-[#3E7BC4]"
        />
          ...
          <table className="w-full text-[13px]">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((inv) => (
                  <tr key={inv.id} onClick={() => setDrawerId(inv.id)} className="cursor-pointer border-b border-[#F5F9FE] last:border-0 hover:bg-[#F5F9FE]">
                    <td className="px-3 py-3 font-medium text-[#171A17]">{inv.status === 'draft' ? '—' : inv.number}</td>
                    <td className="px-2 py-3 text-[#171A17]">{inv.customerName}</td>
                    <td className="px-2 py-3" onClick={(e) => e.stopPropagation()}>
                      <Link href={`/app/orderflow/orders/${inv.orderId}`} className="text-[#1F5FA8] hover:underline">{orderRef(inv.orderId)}</Link>
                    </td>
                    <td className="px-2 py-3"><InvoiceStatusBadge status={inv.status} /></td>
                    <td className="px-2 py-3 text-[#6B6F68]">{inv.status === 'draft' ? '—' : fmtDate(inv.issued)}</td>
                    <td className="px-2 py-3 text-[#6B6F68]">{inv.status === 'draft' ? '—' : fmtDate(inv.due)}</td>
                    <td className="px-2 py-3 text-right tabular-nums text-[#171A17]">{zar(inv.total)}</td>
                    <td className="px-2 py-3 text-right tabular-nums text-[#6B6F68]">{inv.paid > 0 ? zar(inv.paid) : '—'}</td>
        ...
        footer={
          drawer ? (
            <div className="flex items-center justify-between">
              <button type="button" onClick={() => toast('PDF downloaded (demo)')} className="text-[13px] font-medium text-[#6B6F68] hover:text-[#171A17]">Download PDF</button>
              <div className="flex gap-2">
                <button type="button" onClick={() => toast('Reminder sent (demo)')} className="rounded-lg border border-[#E2E6EC] bg-white px-3.5 py-2 text-[13px] font-medium text-[#171A17] hover:border-[#3E7BC4]/40">Send reminder</button>
                {drawer.balance > 0 ? (
                  <button type="button" onClick={() => void markPaid(drawer)} className="rounded-lg bg-[#1F5FA8] px-3.5 py-2 text-[13px] font-medium text-white hover:bg-[#174C87]">Mark paid</button>
                ) : null}
              </div>
            </div>
          ) : undefined
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3 text-[13px]">
        <Field label="Linked order"><Link href={`/app/orderflow/orders/${inv.orderId}`} className="text-[#1F5FA8] hover:underline">{orderRef(inv.orderId)}</Link></Field>
      ...
      <Section title="Line items">
        <table className="w-full text-[13px]">
          <tbody>
            {items.length === 0 ? (
              <tr><td className="py-4 text-center text-[#8A8E86]">No items.</td></tr>
            ) : (
              items.map((i, idx) => (
      ...
      <Section title="Payment history">
        {inv.payments.length === 0 ? (
          <p className="text-[13px] text-[#8A8E86]">No payments recorded yet.</p>
        ) : (
          <div className="flex flex-col gap-2 text-[13px]">
            {inv.payments.map((p) => (
              <div key={p.id} className="flex items-center justify-between">
                <span className="text-[#6B6F68]">{fmtDate(p.date)} · {p.method} · {p.reference}</span>
      ...
      {contact ? (
        <Section title="Customer contact">
          <div className="rounded-xl border border-[#EEF1F5] bg-[#FCFCFB] px-3.5 py-3 text-[13px]">
            <div className="font-medium text-[#171A17]">{contact.name}</div>
            <div className="mt-0.5 text-[#6B6F68]">{contact.email ?? '—'}{contact.phone ? ` · ${contact.phone}` : ''}</div>
          </div>
        </Section>
      ) : null}
          ...
          {activity.map((a) => (
            <div key={a.id} className="flex gap-3 text-[13px]">
              ...
              <div className="min-w-0">
                <div className="text-[#171A17]">{a.label}{a.detail ? <span className="text-[#8A8E86]"> · {a.detail}</span> : null}</div>
                <div className="text-[11px] text-[#8A8E86]">{fmtDate(a.date)}</div>
              </div>
            </div>
          ))}
        </div>
