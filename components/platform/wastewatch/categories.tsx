...
const RADIUS_STYLE = { fontFamily: 'var(--font-instrument)', ['--radius' as string]: '0.625rem' } as React.CSSProperties;

/** Palette offered in the colour picker — each visibly distinct. */
// Exempt from the platform palette sweeps: the swatch a user picks here is
// written to the database, so these are data, not chrome.
...
export function WasteWatchProvider({ data, children }: { data: WasteWatchData; children: ReactNode }) {
  ...
  const value = useMemo<WasteWatchCtx>(() => {
    ...
    return {
      ...
      colorOf: (name: string) => cats.find((c) => c.name === name)?.color ?? '#8A8E86',
...
export function CategoryModal({ open, mode, category, onClose, onSaved }: { open: boolean; mode: 'create' | 'edit'; category?: WasteCategoryRow | null; onClose: () => void; onSaved?: (name: string) => void }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={RADIUS_STYLE}>
      ...
      <div role="dialog" aria-modal="true" className="relative w-full max-w-[400px] rounded-2xl border border-[#EAEDF2] bg-white p-5 shadow-[0_24px_70px_-20px_rgba(26,28,30,0.45)]">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="of-display text-[18px] font-semibold tracking-[-0.015em] text-[#171A17]">{mode === 'edit' ? 'Edit category' : 'New waste category'}</h2>
            <p className="mt-1 text-[13px] text-[#6B6F68]">Group waste your own way — it’ll appear here and in the Log waste form.</p>
          </div>
        ...
        <div className="mt-4 space-y-4">
          <div>
            <label className="mb-1.5 block text-[13px] font-medium text-[#171A17]">Name</label>
            <input autoFocus value={name} onChange={(e) => { setName(e.target.value); if (error) setError(null); }} onKeyDown={(e) => { if (e.key === 'Enter') submit(); }} placeholder="e.g. Leafy Greens" className="h-11 w-full rounded-[12px] border border-[#E4E9F0] bg-white px-4 text-[14px] text-[#171A17] outline-none placeholder:text-[#A0A49C] focus:border-[#3E7BC4]" />
          </div>
          <div>
            <label className="mb-1.5 block text-[13px] font-medium text-[#171A17]">Colour</label>
            <div className="flex flex-wrap gap-2">
              {CATEGORY_SWATCHES.map((c) => (
                <button key={c} type="button" onClick={() => setColor(c)} aria-label={`Colour ${c}`} className={`h-7 w-7 rounded-full transition-transform ${color === c ? 'ring-2 ring-offset-2 ring-[#171A17]/60 scale-110' : 'hover:scale-110'}`} style={{ backgroundColor: c }} />
              ))}
            </div>
          </div>
          <div className="flex items-center gap-2 rounded-[12px] border border-[#EAF2FC] bg-[#F5F9FE] px-3.5 py-2.5">
            <span className="h-3 w-3 rounded-full" style={{ backgroundColor: color }} />
            <span className="text-[13px] text-[#6B6F68]">Preview · <span className="font-medium text-[#171A17]">{name.trim() || 'New category'}</span></span>
          </div>
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button type="button" onClick={submit} disabled={busy} className="inline-flex h-[42px] items-center rounded-[11px] bg-[#1F5FA8] px-[18px] text-[14px] font-semibold text-white transition-colors hover:bg-[#174C87] disabled:opacity-60">{busy ? 'Saving…' : mode === 'edit' ? 'Save changes' : 'Create category'}</button>
        </div>
      </div>
    </div>,
    document.body,
