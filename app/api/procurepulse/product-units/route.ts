...
// ProductUnits.tsx's conversion_factor field now legitimately sends comma
// decimals (post locale-number sweep); malformed input still fails to null,
// never a guess, same as before — the positive guard is unchanged.
const posNum = (v: unknown): number | null => {
  if (v === '' || v == null) return null;
  const n = typeof v === 'number' ? (Number.isFinite(v) ? v : null) : parseLocaleNumber(String(v)); // reads "12,5"; malformed still → null
